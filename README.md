...
- **Civil calendar awareness** — U.S. civil holidays and observances (Mother's Day, Memorial Day, Independence Day, Father's Day, Thanksgiving…) are shown alongside the liturgical day on the Today card and in every date lookup, so they're visible when planning. A holiday that falls on a weekday is also flagged on its **nearest Sunday** (shown lighter, with the actual date — e.g. a Saturday Independence Day surfaces on the Sunday after), since that's the day people gather. Moveable dates are derived programmatically, not hand-typed. They are clearly marked as informational only — never church propers — and can be hidden in Settings or added to the iCal feed with `&civil=1` (the iCal feed lists each holiday on its real date only)
...
- **Almanac** — a suite of calendar and lunar tools (under **Almanac** in the nav), reviving the computational side of the 1997 *Calendar Explorer*: **Moon Phases** (every phase for a year with full/new moon dates, blue moons, and an ecclesiastical-vs-astronomical comparison), **Date of Easter** (Western vs Eastern side by side, with the years they coincide), **Movable Feasts**, **Passover & the Paschal Moon** (the spring full moon reconstructed from 1500 BC to the present), **Easter Statistics**, a **Calendar Converter** (Julian ↔ Gregorian + Julian day number), and a **Help & Guide** page. Computed in pure Python (Meeus); Moon phases match the U.S. Naval Observatory to ~2 minutes, and the 4 BC full moon matches NASA's eclipse catalog to ~8 minutes. Works back to apostolic times (Julian calendar pre-1582); the date of Easter is exact integer arithmetic
- **Historic Lent** — the two ancient ways of keeping the weekdays of Lent (under **Lent** in the nav): the **Historic Lenten Weekday Lectionary** (`/lent/ferial`), the Western rite's proper Mass for *every* day of Lent — Ash Wednesday through Good Friday — for any year, each reading tagged for its Roman or Lutheran provenance (sourced from The Lutheran Missal); every day opens a printable **day card** (`/lent/ferial/<date>`) with a one-page **PDF** (`/lent/ferial/<date>/pdf`). And **The Passion History** (`/lent/passion`), the CSB 1917 harmony of the four Evangelists in seven Parts, readable on a toggle between the Wednesdays of Lent and Holy Week (per the CSB's own rubrics), with KJV links and the historic kneeling rubric. A **sources page** (`/lent/sources`) documents the full Roman-vs-Lutheran history and rubrics with citations. Built on a tradition-scoped data model for future RCL / Roman tracks
...
- **Almanac** ✅ — Moon phases, the date of Easter (West/East), movable feasts, the Passover/Paschal moon back to the Exodus era, Easter statistics, and a Julian/Gregorian converter; validated against USNO and NASA data
- **Civil / secular holiday awareness** ✅ — U.S. civil holidays shown alongside the liturgical day (Today card, date lookup), including a weekday holiday's **nearest Sunday** (e.g. a Saturday Independence Day on the Sunday after); togglable in Settings and addable to the iCal feed with `&civil=1`; informational only, kept distinct from church propers. Other countries to follow
...
### Almanac — *Calendar Explorer* revivals (ranked easiest → hardest)

Computational features from Albert's own 1997 *Calendar Explorer 1.86* (shareware; help file recovered June 2026) that the Almanac doesn't yet have. Ordered by implementation difficulty:

1. **Weekday Finder** ✅ — day-of-week for any date (with day-of-year, ISO week, leap-year), plus "when does this date next fall on a chosen weekday" (birthday-on-a-Sunday finder). At `/almanac/weekday`.
2. **Date Calculator** ✅ — days between two dates (weeks + remainder, and a Mon–Fri weekday count) and a date ± N days. At `/almanac/datecalc`.
3. **Islamic (Hijri) civil calendar converter** — Gregorian → Hijri civil date via the deterministic tabular ("civil") algorithm; bolts onto the existing Julian/Gregorian Calendar Converter. Arithmetic-only; validate against a reference table.
4. **Indian (Śaka) civil calendar converter** — Gregorian → Indian national civil date; deterministic arithmetic (per the *Explanatory Supplement to the Astronomical Almanac*, already in CE's bibliography). Slightly fiddlier leap rule than Hijri.
5. **"Calendar Properties" day report** — one panel showing a single date's Gregorian, Julian Day Number, Julian-calendar, Hijri, and Śaka values together. Mostly composition once #3/#4 land (JDN + Julian already exist).
6. **"Year in Brief" 12-month grid** — all twelve mini-months of a year on one page, optionally shaded by liturgical season; reuses the existing season-coloring engine but needs a new print/PDF layout.
7. **Patristic Easter reader** — static reference pages of the primary sources on the Quartodeciman/Nicene Easter controversy (Hippolytus, Eusebius' *Life of Constantine*, the Nicene Synodal Letter, Hefele, Canons of Antioch). No computation — text already recovered from the CE help file — but real transcription/citation-verification effort (per the never-hand-type-liturgical-data rule).
8. **Roman Catholic fixed feasts (199)** — the non-movable General Roman Calendar sanctoral. Data-sourcing and cross-verification heavy, must come from a published authority (not hand-typed), and raises a scope question for a Lutheran project. CE paired this with its movable Three-Year report. *Hardest.*
